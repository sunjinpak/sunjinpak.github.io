---
layout: default
title: "Project 6: Leader Workaholism & Food Safety"
---

# Project 6: Leader Workaholism & Food Safety

**Title**: The Cost of the Grind: Assessing the Impact of Leader Workaholism on Food Safety Compliance and Employee Retention

**Team**: Eric

**Competition**: Grimm Family Center Student Research Symposium

---

## Overview

This study examines how leader workaholism affects food safety compliance and employee retention in the agriculture and food processing industry. Drawing on Conservation of Resources (COR) theory, the project investigates whether supervisors who exhibit workaholic behaviors create conditions that deplete employee resources, leading to emotional exhaustion, reduced food safety compliance, and higher turnover intentions. The study uses an online survey targeting 100-150 employees in the agriculture and food processing sector.

---

## 1. Paper Writing

### Resources

- [Quantitative Research Paper Guide](quantitative-research-paper-guide) - How to write a quantitative research paper in APA format
- [Literature Review Guide](literature-review-guide) - How to conduct a literature review

### Paper Structure

Your final paper should include:

1. **Title Page** - Title, authors, affiliation
2. **Abstract** - 150-250 words summarizing your study
3. **Introduction** - Literature review and hypotheses
4. **Method** - Participants, procedure, measures, analytic strategy
5. **Results** - Preliminary analyses and hypothesis tests
6. **Discussion** - Interpretation, implications, limitations, future directions
7. **References** - APA 7th edition format
8. **Tables** - Correlation table, regression results

### What You Need to Write

| Section | Status |
|---------|--------|
| Title Page | To do |
| Abstract | To do |
| Introduction | To do |
| Method | To do |
| Results | To do |
| Discussion | To do |
| References | To do |

### Method Section Guide

Your Method section should include four subsections. Here is what to write in each.

#### Participants and Procedure

- Describe who participated (employees in the agriculture and food processing industry, aged 18+)
- Mention the recruitment method (online survey distributed via LinkedIn and snowball sampling)
- Report the target sample size (100-150) and the actual number of responses you received
- Note that participation was voluntary and anonymous, with IRB approval (Protocol #27-12)
- Mention informed consent was obtained at the start of the survey
- Report demographics (age, gender, tenure, job type, industry sector, shift, education level) with means, SDs, and percentages

#### Measures

For each measure, write a brief paragraph that includes the source, number of items, sample item, response scale, and Cronbach's alpha from your data. Example format:

> **Leader Workaholism.** Leader workaholism was measured using a 10-item adapted version of the Dutch Work Addiction Scale (DUWAS; Schaufeli et al., 2009). Participants rated their direct supervisor on a 5-point Likert scale (1 = *Strongly Disagree*, 5 = *Strongly Agree*). The scale has two subscales: Working Excessively (5 items) and Working Compulsively (5 items). A sample item is "My supervisor stays busy and keeps many irons in the fire." Cronbach's alpha was [report from your data].

Write similar paragraphs for:
- **Food Safety Compliance** (6 items, 5-point scale from *Never* to *Always*, adapted from Neal & Griffin, 2006)
- **Turnover Intention** (3 items, 5-point Likert scale, Colarelli, 1984; note one item is reverse-coded)
- **After-Hours Work Contact** (5 items, 5-point scale from *Never* to *Very Often*, adapted from Derks et al., 2015)
- **Emotional Exhaustion** (5 items, 5-point scale from *Never* to *Always*, Maslach & Jackson, 1981)
- **Job Satisfaction** (3 items, 5-point Likert scale, Cammann et al., 1983; note one item is reverse-coded)

#### Control Variables

- List any demographic variables you control for in your analyses (e.g., age, tenure, education)

#### Analytic Strategy

- Correlation analysis to examine relationships among all study variables
- Multiple regression to test H1-H3 (direct effects)
- Mediation analysis using the PROCESS macro (Hayes, 2022, Model 4) to test H4-H5
- Report that you checked for normality, multicollinearity, and common method bias

### Writing Tips

1. Start with the **Introduction** - build the theoretical foundation for your hypotheses
2. Use the [Quantitative Research Paper Guide](quantitative-research-paper-guide) for APA formatting
3. Keep your writing clear and concise
4. Use COR theory as your primary theoretical framework

---

## 2. Survey

**Survey Link**: [https://food-safety-survey.netlify.app](https://food-safety-survey.netlify.app)

### Target Participants

- Employees aged 18+ in the agriculture or food processing industry
- Target sample size: 100-150 participants
- Recruitment: LinkedIn and snowball sampling

### Survey Measures

| Measure | Items | Source |
|---------|-------|--------|
| Leader Workaholism | 10 | Dutch Work Addiction Scale (Schaufeli et al., 2009) |
| Food Safety Compliance | 6 | Safety Compliance Scale (Neal & Griffin, 2006) |
| Turnover Intention | 3 | Colarelli (1984) |
| After-Hours Work Contact | 5 | Derks et al. (2015) |
| Emotional Exhaustion | 5 | Maslach Burnout Inventory |
| Job Satisfaction | 3 | Michigan Organizational Assessment Questionnaire |
| Demographics | 7 | Age, gender, tenure, job type, industry sector, shift, education |

---

## 3. Research Questions

- **H1**: Leader workaholism is negatively related to employee food safety compliance.
- **H2**: Leader workaholism is positively related to employee turnover intention.
- **H3**: After-hours work contact is positively related to emotional exhaustion.
- **H4**: Emotional exhaustion mediates the relationship between leader workaholism and turnover intention.
- **H5**: Job satisfaction mediates the relationship between leader workaholism and turnover intention.

---

## 4. Presentation

See [Research Presentation Guide](research-presentation-guide) for detailed instructions.

**Key Info:**
- **Presentation**: 20 minutes
- **Q&A**: 5 minutes

---

## 5. Timeline

### Grimm Family Center Student Research Symposium

| Task | Deadline |
|------|----------|
| Begin data collection | March 20, 2026 (Fri) |
| Draft Introduction | March 28, 2026 (Sat) |
| Draft Method | April 1, 2026 (Wed) |
| Draft Results & Discussion | April 4, 2026 (Sat) |
| Prepare presentation | April 5-9, 2026 |
| **Symposium Presentation** | **April 10, 2026 (Fri) 10:00 AM - 1:30 PM** |

- **Format**: 20 minutes presentation + 5 minutes Q&A
- **Location**: Walter W. Stiern Library, Dezember Reading Room

---

## 6. Questions?

Contact Dr. Pak if you have any questions about:
- Paper writing
- Data collection
- Data analysis
- Presentation preparation

---

[← Back to Home](.)
